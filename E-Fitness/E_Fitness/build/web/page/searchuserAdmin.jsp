...
<%
    try{
        ...
        String where = "";
        if(!type.equals("")){
            where = " WHERE " + type + " LIKE '" + request.getParameter(type)  + "%' ";
        }
        ...
        ResultSet rs = myBean.selectData("SELECT * FROM user " + where + " ORDER BY " + sidx + " " +sord + " LIMIT " + (int)start + "," + (int)limit);
        rs.last();



        ArrayList param2 = new ArrayList();
        ResultSet rs2 = myBean.selectData("SELECT * FROM user " + where);
        rs2.last();
